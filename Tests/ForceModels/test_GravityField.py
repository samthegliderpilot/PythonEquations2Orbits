...
#     subsDict[sy.cos(lon)] = fixedPositionVectorNorm[0] / (sy.cos(lat))
#     subsDict[sy.sin(lon)] = fixedPositionVectorNorm[1] / (sy.cos(lat))
#     subsDict[sy.Abs(sy.cos(lat))]=sy.cos(lat)
#     subsDict[lat] = sy.asin(fixedPositionVectorNorm[2])
#     subsDict[lon] = sy.atan2(sy.sin(lon), sy.cos(lon))
#     #subsDict[sy.Abs(sy.sin(lat))]=sy.sin(lat)
...
#     delUDelR =  (-mu/(rSy**2))*sy.summation(  sy.summation( ((rCbSy/rSy)** n)*(n+1)*sy.assoc_legendre(n, m, sy.sin(lat))*(cFunc(n,m)*sy.cos(m*lon) + sFunc(n,m)*sy.sin(m*lon)), (n, 2, nMaxSy)), (m, 0, n) )
#     delUDelLat =  (mu/rSy)*sy.summation(  sy.summation( ((rCbSy/rSy)** n)*(sy.assoc_legendre(n, m+1, sy.sin(lat)) -m*sy.tan(lat))*(cFunc(n,m)*sy.cos(m*lon) + sFunc(n,m)*sy.sin(m*lon)), (n, 2, nMaxSy)), (m, 0, n) )
#     delUDelLon =  (mu/rSy)*sy.summation(  sy.summation( ((rCbSy/rSy)** n)*m*(sy.assoc_legendre(n, m, sy.sin(lat)))*(sFunc(n,m)*sy.cos(m*lon) - cFunc(n,m)*sy.cos(m*lon)), (n, 2, nMaxSy)), (m, 0, n) )
