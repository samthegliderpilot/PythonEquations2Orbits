...
if __name__ == "__main__":
    ...
    modules = [{"cFunc": getC, "sFunc": getS, "assoc_legendre": betterAssoc}, "numpy"]
...
def _lambdifygenerated(r_c, r, lat, lon, n_max, m_max):
    return (builtins.sum((r_c/r)**n[n]*(cFunc(n[n], m[m])*cos(lon*m[m]) + sFunc(n[n], m[m])*sin(lon*m[m]))*(n[n] + 1)*sy.assoc_legendre(n[n], m[m], sin(lat)) for m in range(0, m_max+1) for n in range(2, n_max+1)))
