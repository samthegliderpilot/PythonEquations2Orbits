...
class testOneDWorkSymbolicProblem(unittest.TestCase) :
    def testBoilerplate(self) :
        ...
        self.assertEqual(2, len(problem.StateSymbols), msg="count of state variables")
        self.assertEqual(1, len(problem.ControlSymbols), msg="count of control variables")
        self.assertEqual(problem.ControlSymbols[0]**2, problem.UnIntegratedPathCost, msg="unintegrated path cost")
        ...
        self.assertEqual(2, problem.StateVariableDynamics[0].subs(problem.StateSymbols[1], 2), msg="eom 1")
        self.assertEqual(3, problem.StateVariableDynamics[1].subs(problem.ControlSymbols[0], 3), msg="eom 2")
        self.assertEqual(0, problem.BoundaryConditions[0].subs(problem.StateSymbols[0].subs(problem.TimeSymbol, problem.TimeFinalSymbol), 1), msg="bc1")
        self.assertEqual(1, problem.BoundaryConditions[1].subs(problem.StateSymbols[1].subs(problem.TimeSymbol, problem.TimeFinalSymbol), 1), msg="bc2")
