...
def TrapezoidalForward(callbackTThenState, y0 :npt.NDArray, t0 : float, tNext :float, parametersArray : Optional[List[float]] = None) -> List[float] :
    dy0 = np.array(callbackTThenState(t0, y0, parametersArray))
    dt = tNext - t0
    return y0 + dt*dy0

def TrapezoidalBackward(callbackTThenState, y0 : npt.NDArray, t0 : float, tNext :float, parametersArray : Optional[List[float]] = None) -> List[float] :
    dt = tNext - t0
    def fSolveCallback(y1) :
        return y0+ dt*np.array(callbackTThenState(tNext, y1, parametersArray)) - y1

    forwardGuess = TrapezoidalForward(callbackTThenState, y0, t0, tNext, parametersArray)
...
allDataBackward = integrateSystemOverTimeRange(callback, TrapezoidalBackward, initialValues, [g],t0, tf, count)
